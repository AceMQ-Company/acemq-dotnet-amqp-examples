# intermediate/07 — a producer on a new schema and a consumer on an old one, in C#
...
[intermediate/07-schema-evolution-vbnet](../07-schema-evolution-vbnet).
...
**A codec belongs to a connection.** The `Publisher<T>` overload that takes an
