# intermediate/08 — a pipeline, and resuming one mid-run, in C#
...
[intermediate/08-pipelines-vbnet](../08-pipelines-vbnet).
...
```
pipeline    dotnet-csharp-fulfilment: validate -> charge -> ship
...
pipeline    dotnet-csharp-fulfilment-by-address: validate -> ship
slip form   Itinerary, in acemq-routing-slip
on the wire acemq-routing-slip: {"steps":[{"exchange":"","routingKey":"dotnet-csharp-fulfilment-by-address.ship","name":"ship"}],"done":[{"exchange":"","routingKey":"dotnet-csharp-fulfilment-by-address.validate","name":"validate","completedAt":"..."}]}
stalled at  ship on dotnet-csharp-fulfilment-by-address.ship, after validate
...
A step returning `null` ends the message's journey there, which is how a filter
is expressed: being rejected by a validation step is a normal outcome and a
failure is not, so they are counted apart (`EndedEarly` against `Completed`).
...
```csharp
await mq.DeclareQueueAsync(pipeline.QueueFor("charge") + ".dlq");   // don't
```
