# intermediate/04 — a saga, in VB.NET
...
The same example exists in C# at [intermediate/04-saga-csharp](../04-saga-csharp).
...
## A step that fails, in VB.NET

A saga step is a `Func(Of T, Task)`, so a step that refuses has to *produce* a
failed task rather than throw out of a lambda:
...
A VB lambda declared `As Task` must return one on every path, and an `Async`
lambda with no `Await` in it is a warning — which this repository builds as an
error. `Task.FromException` is the shape that is neither.

The other VB.NET trap is the usual one: `Envelope` is a type, so the variable
holding one is called `stamp`. VB.NET is case-insensitive, and the compiler
reports the collision as a type it cannot infer rather than as a name clash.
...
That is not a defect in `Saga(Of T)`; it is what compensating a real-world action
...
Nothing in `Saga(Of T)` publishes anything and no header is set, so unlike the
