# advanced/02 — a claim check, in VB.NET
...
[advanced/02-claim-check-csharp](../02-claim-check-csharp).
...
```
payloads go to /tmp/acemq-claim-check-vbnet
...
the store holds 1 payload(s): 5557783f-0d46-4b20-af05-bfa9215238f5
...
**`framing`, not `codec`.** VB.NET is case-insensitive, so a variable sharing a
type's name is reported as a type that cannot be inferred rather than as a name
clash — the same caution that names a keyring `ring` in
[advanced/01](../01-encrypting-payloads-vbnet).

**`System.IO.Directory`, spelled out.** The local variable holding the store's
path is called `directory`, which shadows the `Directory` type inside this
method; qualifying the two calls that need the type is shorter than renaming the
variable to something less accurate.

Reading raw bytes off a queue also needs the array spelled into the type
argument:

```vb
Await mq.ConsumeAsync(Of Byte())(
