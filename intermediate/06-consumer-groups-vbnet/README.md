# intermediate/06 — several consumers over one queue, in VB.NET
...
[intermediate/06-consumer-groups-csharp](../06-consumer-groups-csharp).
...
```
1 consumer  40 jobs in 2162 ms, peak concurrency 1
8 consumers 40 jobs in 293 ms, peak concurrency 8
...
**A number.** The size is an `Integer`, so it comes from configuration. It is the
...
## Three things VB.NET makes you name differently

**`workers`, not `group`.** `Group` is a query keyword and would have to be
written `[group]` to be a variable at all.

**`eightAtOnce`, not `parallel`.** VB.NET is case-insensitive, so a variable
called `parallel` collides with `System.Threading.Tasks.Parallel` — reported as a
type that cannot be inferred rather than as a name already in use.

**`AddressOf Handle`, not a lambda.** The handler is passed by name here because
`ConsumerGroup.StartAsync(Of Job)` takes a `Func(Of IMessage(Of Job), Task(Of
Ack))`, and `AddressOf` on a declared `Async Function` binds to it without the
lambda having to spell its own signature out.
