# basic/07 — a log that is read rather than emptied, in VB.NET
...
[basic/07-streams-csharp](../07-streams-csharp).

## What it shows

- **Retention is the argument that matters.** `DeclareStreamAsync` takes
  `Nothing` for both limits, which is legal and almost always wrong.
...
```vb
Await mq.DeclareStreamAsync(OrderLog, TimeSpan.FromHours(1), 20L * 1024 * 1024, 1L * 1024 * 1024)
```
...
```vb
checkpoint = readerOne.LastHandledOffset.Value   ' the offset handled, not the count
' ... later, in another process
mq.Stream(Of Order)(OrderLog).FromOffset(checkpoint + 1)
```
...
`LastHandledOffset` is a `Long?`, and it is `Nothing` until a message has been
handled. With `Option Strict On` that has to be unwrapped deliberately, which is
the point: a reader that handled nothing has no checkpoint, and defaulting it to
zero would silently mean "start from the beginning".
...
**`OrderLog`, not `Log`.** VB.NET is case-insensitive, and a member called `Log`
collides with `Microsoft.VisualBasic`'s `Log` function — reported as a call that
cannot be resolved rather than as a name already in use.

**The handler needs its parameter typed.** `Function(message As IMessage(Of
Order)) As Task` rather than `Function(message)`: with `Option Strict On` the
lambda has to say what it takes and returns before the overload it is being
passed to can be chosen.

`mq.Stream(Of Order)(...)` is safe despite `System.IO.Stream` existing, because
it is a member access on the connection rather than a bare name.
...
is a queue, and
[intermediate/06-consumer-groups-vbnet](../../intermediate/06-consumer-groups-vbnet)
